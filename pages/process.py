...
column1 = dbc.Container([ 
		...
		html.I(['Note: I did not use a baseline mode based upon the average GHI. In this situation creating a model on limited data can still provide a pretty good R² score and Mean Absolute Error. A mean baseline model would not be useful for this project because even a really bad predictive model would still beat the mean baseline. It is better to create a baseline model with only a few features and then improve upon said model by adding more features that correlate with the target.']),
		html.Br([]),
		html.Br([]),
		...
		html.P(['I‌ then created my final model with this new dataset and got great results. My R² score increased to 0.923 and my Mean Absolute Error decreased to 53.5. I tested this model on data from various locations and consistently received nealy identical scores.']),
		html.Br([]),
		html.Img(src='assets/fin2.png', className='img-fluid',style={'textAlign':'center','margin':'auto'}),
		html.Br([]),
		html.Br([]),
